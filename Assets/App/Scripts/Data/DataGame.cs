...
public class DataGame : ScriptableObject
{
    ...
    public ScenarioLocation[] scenarioLocations = new ScenarioLocation[]
    {
        // Сценарий 1
    ...
    private string rules =
        $"Переживи их всех - это социальная игра про группу людей, которая должна выжить в нестандартной ситуации. " +
        $"К сожалению, не всем суждено спастись, поэтому игрокам предстоит делать нелегкие выборы кто доживет до конца, " +
        $"основываясь на возможностях игровых персонажей.\n\n" +
        $"В игровой сессии присутствуют от 4 до 8 игроков. Игровая сессия делится на 4 игровых раунда, " +
        $"во время которых игроки раскрывают информацию о себе и доказывают свою важность для остальных. " +
        $"Также во время раундов могут происходить события, которые могут перевернуть ход игры с ног на голову. " +
        $"В конце раунда проводится голосование, по итогу которого исключается 1 из игроков. Когда заканчивается последний раунд, " +
        $"игроки получают одну из концовок. На концовку влияет Рейтинг Выживаемости и выполнение специального условия.\n\n" +
        $"Ход игры: \n" +
        $"1. Создается игра и настраивается кол-во игроков.\n" +
        $"2. Игроки поочередно смотрят информацию о своих персонажах и безопасной комнате, после передают ход следующему игроку.\n" +
        $"3. После того, как последний игрок нажимает кнопку “Закончить ход” - начинается 1 раунд\n" +
        $"4. Все раунды проходят по одной схеме:\n" +
        $"   a. Запускается таймер раунда\n" +
        $"   b. Происходит Случайное Событие\n" +
        $"   c. Раскрывается 1 характеристика каждого из персонажей (раскрываются только одинаковые характеристики)\n" +
        $"   d. Игроки обсуждают ситуацию и доказывают важность своего персонажа\n" +
        $"   e. Когда таймер заканчивается - начинается голосование за исключение одного из игроков\n" +
        $"5. После истечения всех раундов игра заканчивается, игроки получают одну из концовок\n\n" +
        $"Рейтинг выживаемости:\n" +
        $"Каждая особенность персонажа или предмет в игре влияют на Рейтинг Выживаемости. " +
        $"Это мерило того, какие шансы у оставшихся игроков на успешное выживание. " +
        $"Рейтинг может быть как положительным, так и отрицательным.\n\n" +
        $"Персонажи:\n" +
        $"Аватары игроков в сценарии, обладают полезными (и не очень) особенностями и предметами. " +
        $"Даже если вам выпали не самые лучшие черты - у вас всегда есть шанс - конец света это еще не конец!\n\n" +
        $"Специальные условия:\n" +
        $"У каждого персонажа в игре есть своя скрытая роль (игроки видят только свои скрытые роли и " +
        $"могут только догадываться о ролях остальных игроков). " +
        $"Для получения особенной концовки необходимо выполнить специальное условие " +
        $"(у каждого сценария свое условие).\n\n" +
        $"Скрытая роль:\n" +
        $"У каждого персонажа в игре есть своя скрытая роль, которая добавляет глубину для отыгрыша и новые цели.\n\n" +
        $"На данный момент есть 2 роли:\n\n" +
        $"Человек - бедолага, оказавшийся не в том месте и не в то время, главная задача - выжить.\n\n" +
        $"Мимик/Одержимый - цель персонажей с этой  ролью выгнать как можно больше Людей и остаться в команде выживших.\n\n";
    
    public int ScenarioCount { get; set; }
    public Scenario Scenario { get; set; }
    public ScenarioLocation ScenarioLocation{ get; set; }
    ...
    public List<PlayerUI> Players => _players;

    private List<PlayerUI> _players = new List<PlayerUI>();
    ...
    private Scenario GetScenario() { return scenarios.FirstOrDefault(t => t.number == ScenarioCount); }
    private ScenarioLocation GetScenarioLocation() => scenarioLocations.GetRandomByScenario(ScenarioCount, p => p.scenario);
    public GameEvent GetGameEvent() => gameEvents.GetRandomByScenario(ScenarioCount, p => p.scenario);
    public Ending GetEnding(int rv) {
        Ending ending = null;
        for (int i = 0; i < endings.Length; i++) {
            if (endings[i].scenario == ScenarioCount) {
                ending = endings[i];
                if (rv >= endings[i].value) {
                    ending = endings[i];
                }
                else {
                    break;
    ...
    {
        ScenarioCount = Random.Range(1, maxScenario + 1);
